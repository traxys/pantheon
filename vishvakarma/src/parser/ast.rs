...
pub enum Expression {
    ...
    Field {
        source: Rc<SpannedValue<Expression>>,
        field: Rc<str>,
    },
}
...
impl Expression {
    ...
    fn parse(
        ...
        tokens.parse_seq(|tokens| {
            let expr =
                tokens.parse_seq(|tokens| Self::parse_base_expr(tokens, root, directives))?;

            let followed_expr = tokens.parse_seq(|tokens| {
                let mut peeker = tokens.peek_cursor();
                match peeker.peek() {
                    Ok(TokenKind::Dot) => {
                        tokens.next().unwrap();
                        let field = tokens.next_identifier("field")?;

                        Ok::<_, ParseError>(Expression::Field {
                            source: Rc::new(expr),
                            field: Rc::from(field.v),
                        })
                    }
                    _ => Ok(expr.v),
                }
            ...
            match peeker.peek() {
                Ok(TokenKind::Plus) => {
                    ...
                    Ok(Expression::Sum {
                        lhs: Rc::new(followed_expr),
                        rhs: Rc::new(rhs),
                    })
                }
                _ => Ok(followed_expr.v),
            }
        })
    }
}
